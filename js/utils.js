...
var dMons = [  8,  8,  8,  9,  9,  9,  9, 10, 10, 10, 10, 11, 11, 11, 11, 11 ];
var dDays = [ 16, 23, 30,  7, 14, 21, 28,  4, 11, 18, 25,  2,  9, 16, 23, 30 ];


function CalcWeek(today)
{
   var daynum = getDOY(today);

   var tuesday;
   var tuesnum;

   var week = 1;
   for ( var indx=0; indx<dMons.length; indx++ )
   {
      var d = new Date();
      var year = d.getFullYear();
      tuesday = new Date(year, dMons[indx], dDays[indx]);
      tuesnum = getDOY(tuesday); 
...
function bsort(names, wins, losses)
{
